...
class GeneralizedRCNN(nn.Module):
    ...
    def __init__(self, cfg):
        ...
        # --- TSNE Feature Collection Setup ---
        # Buffer to store (feature_vector, label) tuples. Initialized in __init__ 
        # to prevent AttributeError.
        self.tsne_feature_buffer = []
        # Control flag accessed by inference_on_dataset
        self.collect_tsne_features = False
        self.tsne_collection_limit = 5000 # Stop collecting after this many samples
    ...
    def forward(self, batched_inputs):
        ...
        # _forward_once_ returns 6 values
        proposal_losses, detector_losses, _, box_features, gt_classes, _ = self._forward_once_(batched_inputs, gt_instances)
        
        # Collect features in training mode if explicitly enabled (e.g., for monitoring)
        if self.collect_tsne_features and self.training:
             self._collect_tsne_features(box_features, gt_classes)
    ...
    def inference(self, batched_inputs):
        ...
        # Unpack the 6 values returned by _forward_once_
        _, _, results, box_features, gt_classes, image_sizes = self._forward_once_(batched_inputs, None)
        
        # --- Feature collection during inference (if collecting) ---
        if self.collect_tsne_features:
            self._collect_tsne_features(box_features, gt_classes)
    ...
    def _collect_tsne_features(self, box_features, gt_classes):
        """Collects features and labels into the buffer."""
        if not self.collect_tsne_features or len(self.tsne_feature_buffer) >= self.tsne_collection_limit:
            return

        if box_features is not None and gt_classes is not None:
            # Ensure features are flattened and detached from the computation graph
            features_flat = box_features.detach().cpu().numpy()
            if features_flat.ndim > 2: # Flatten pooled features if necessary (e.g., from (N, C, 7, 7) to (N, C*49))
                 features_flat = features_flat.reshape(features_flat.shape[0], -1)

            labels = gt_classes.detach().cpu().numpy()
            
            for feature, label in zip(features_flat, labels):
                if len(self.tsne_feature_buffer) < self.tsne_collection_limit:
                    self.tsne_feature_buffer.append((feature, label))
                else:
                    self.collect_tsne_features = False # Stop collection
                    print(f"Feature collection limit ({self.tsne_collection_limit}) reached.")
                    break
            print(f"Collected {len(self.tsne_feature_buffer)} / {self.tsne_collection_limit} features so far.")


    def run_tsne_visualization(self, num_classes, save_path="tsne_visualization.png"):
        """
        Performs t-SNE on the collected features and plots the results.

        Args:
            num_classes (int): Total number of classes (needed for color mapping).
            save_path (str): Path to save the output PNG file.
        """
        if not self.tsne_feature_buffer:
            print("TSNE feature buffer is empty. Cannot run visualization.")
            return

        print(f"Starting t-SNE computation on {len(self.tsne_feature_buffer)} features...")

        # Separate features and labels
        features = np.array([f for f, l in self.tsne_feature_buffer])
        labels = np.array([l for f, l in self.tsne_feature_buffer])
        ...
        # Determine unique classes present in the collected data
        ...
        for i, class_id in enumerate(unique_labels):
            ...
            # Normalize class_id for the colormap
            color = cmap(class_id / num_classes) 
            
            plt.scatter(tsne_results[indices, 0], tsne_results[indices, 1], 
                        c=[color], label=f'Class {class_id}', alpha=0.6, s=10)

        plt.title(f"t-SNE Visualization of Box Features ({len(self.tsne_feature_buffer)} Samples)")
        ...
        print(f"t-SNE visualization saved to {os.path.abspath(save_path)}")
        # --- End Methods for TSNE ---
