...
class GeneralizedRCNN(nn.Module):
    ...
    def __init__(self, cfg):
        ...
        nsh_cfg = getattr(cfg.MODEL, "NSH", None)
        if nsh_cfg is not None:
            fhm_train_flag = bool(nsh_cfg.get("FHM_TRAIN", True))
            if not fhm_train_flag:
                if hasattr(self.roi_heads, "fhm"):
                    try:
                        self.roi_heads.fhm.eval()
                        for p in self.roi_heads.fhm.parameters():
                            p.requires_grad = False
                        print("FHM found in roi_heads -> set to eval() and froze parameters (FHM_TRAIN=False).")
                    except Exception as e:
                        logging.warning("Failed to freeze FHM: %s", e)
                else:
